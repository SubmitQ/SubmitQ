            ...
            $.ajax({
                ...
                success: function(msg){
                    $('#email_status').ajaxComplete(function(event, request, settings){
                        if (msg == 'OK'){
                            valid3 = 1;
                            $('#email_status').html('&nbsp;&nbsp;<img src="http://harlin.scripts.mit.edu/submitq/images/check.png" align="absmiddle">');
                        }
                        else {
                            valid3 = 0;
    ...
    $('#id').keyup(function(){
        ...
        if (isNumeric(id) && id.length == 9){
            $.ajax({
                ...
                data: "id="+ id,
                success: function(msg){
                    $('#prof_id_status').ajaxComplete(function(event, request, settings){
                        if (msg == 'OK'){
                            valid6 = 1;
                            $('#prof_id_status').html('&nbsp;&nbsp;<img src="http://harlin.scripts.mit.edu/submitq/images/check.png" align="absmiddle">');
                        }
                        else {
                            valid6 = 0;
                            $('#prof_id_status').html(msg);
                        }
                        checkFields(valid1, valid2, valid3, valid4, valid5, valid6);
                    })
                }
            ...
            $('#prof_id_status').html('&nbsp;&nbsp;<font color="red">Invalid</font>');
        }
        checkFields(valid1, valid2, valid3, valid4, valid5, valid6);
    })
...
function checkFields(valid1, valid2, valid3, valid4, valid5, valid6) {
    if (valid1 == 1 && valid2 == 1 && valid3 == 1 && valid4 == 1 && valid5 == 1 && valid6 == 1) {
        $("#register_prof").removeAttr('disabled'); //enable submit button
    }
    else {
        $("#register_prof").attr('disabled', 'disabled'); //disable submit button
    }
}